<script lang="ts">
	...
	onMount(() => {
		...
		return () => {
			...
			inputState.paintTime = Math.floor(timeDiff / 1000);
...
{#if showUI}
	...
	>
		...
		<div
			class="mt-10 w-40 bg-cover bg-no-repeat center-content"
...
<div
	class="pointer-events-none bg-contain bg-center bg-no-repeat full-screen center-content"
	style:background-image="url(/cargoes/{inputState.cargoKind}_canva.webp)"
></div>

{#if showManual}
	<button
		transition:fade
		class="z-[2000] bg-black/60 px-8 full-screen center-content"
		ontouchstart={noDraw}
